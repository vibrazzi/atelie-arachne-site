      ...
      <div className="container px-8 sm:px-16 pb-8 sm:pb-0">
        <Slider {...settings}>
          {ImageList.map(({ id, img, title, description }) => (
            <div
              key={id}
              className="grid grid-cols-1 sm:grid-cols-2 items-center gap-12 sm:gap-24 text-center sm:text-center"
            >
              {/* Imagem centralizada */}
              <div className="flex justify-center items-center">
                <img
                  src={img}
                  alt={title}
                  className="max-w-[350px] sm:max-w-[450px] w-full object-cover rounded-lg shadow-lg transition-transform duration-500 group-hover:scale-110"
                />
              </div>

              {/* Texto centralizado */}
              <div className="flex flex-col justify-center items-center sm:items-center sm:pl-6">
                <h1 className="text-3xl sm:text-5xl font-bold tracking-wide mt-4 sm:mt-6">{title}</h1>
